...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    $periodicalsTitle = $_POST['title'];
    $periodicalsType = $_POST['type'];
    $periodicalsCategory = $_POST['category'];
    $periodicalsAuthor = $_POST['author'];
    $periodicalsDesc = $_POST['desc'];

    try {
        $query = "INSERT INTO periodicals 
        (periodicalsTitle, periodicalsType,periodicalsCategory,periodicalsAuthor, periodicalsDesc)
        VALUES 
        (:periodicalsTitle,:periodicalsType,:periodicalsCategory,:periodicalsAuthor,:periodicalsDesc);
        ";
        ...
        for ($i = 0; $i < count($_FILES['files']['name']); $i++) {
            ...
            $stmt2->bindParam(':periodical_id', $lastId);
