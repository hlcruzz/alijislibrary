...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    $query = "SELECT *,GROUP_CONCAT(guideline_rules.id) as rules_id , GROUP_CONCAT(guideline_rules.guideline_rules_txt SEPARATOR '\n') AS rules_txt FROM guidelines INNER JOIN guideline_rules ON guidelines.id = guideline_rules.guideline_id WHERE guidelines.guidelineName = :guidelineName AND guideline_rules.status = 1 GROUP BY guidelines.guidelineName;";
