...
$("#seePass").on("click", function () {
  const passwordInput = $("#accountPword");
  $(this).hide();
  $("#unseePass").show();
...
$("#editSeePass").on("click", function () {
  const passwordInput = $("#editAccountPword");
  $(this).hide();
  $("#EditUnseePass").show();

  passwordInput.attr("type", "text");

  $("#EditUnseePass").on("click", function () {
    $(this).hide();
    $("#editSeePass").show();
    passwordInput.attr("type", "password");
  });
...
$(document).on("click", "#editGalleryBtn", function () {
  const checkboxCont = $(".checkboxCont");
  const checkboxImage = $(".checkboxImage");
  const result = num++ % 2;
...
$(document).on("click", ".checkboxImage", function () {
  const checkboxImage = $(this);
  const deleteGalleryBtn = $("#deleteGalleryBtn");

  if ($(".checkboxImage:checked").length > 0) {
    deleteGalleryBtn.show();
  } else {
    deleteGalleryBtn.hide();
  }
});
...
tinymce.init({
  selector: "#servicesTxt, #editServicesTxt, #aboutTextarea, #sectionTxt, #editSectionTxt, #newsMsg, #editNewsMsg",
  width: "100%",
  height: "500px",
  plugins: [
    "advlist",
...
$("#socialIcon").on("change", function () {
  const selectedValue = $(this).val();
  $("#displayIcon").empty();
  $("#displayIcon").append(`<i class='${selectedValue}'></i>`);
});

$("#editSocialIcon").on("change", function () {
  const selectedValue = $(this).val();
  $("#editDisplayIcon").empty();
  $("#editDisplayIcon").append(`<i class='${selectedValue}'></i>`);
});
$("#aboutImg").on("change", function () {
  const file = this.files[0];
  const reader = new FileReader();

  reader.onload = function (e) {
    $("#aboutImgAdmin").attr("src", e.target.result);
  };

  reader.readAsDataURL(file);
});

$("#editSectionImg").on("change", function () {
  const file = this.files[0];
  const reader = new FileReader();

  reader.onload = function (e) {
    $("#editSectionImgPrev").attr("src", e.target.result);
  };
...
$("#accountImg").on("change", function () {
  const file = this.files[0];
  const reader = new FileReader();

  reader.onload = function (e) {
    $("#accountImgPrev").attr("src", e.target.result);
  };

  reader.readAsDataURL(file);
});

$("#editAccountImg").on("change", function () {
  const file = this.files[0];
  const reader = new FileReader();

  reader.onload = function (e) {
    $("#editAccountImgPrev").attr("src", e.target.result);
  };
